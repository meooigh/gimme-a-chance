...
const getAllAccounts = async function (req, res, next) {
  ...
  try {
    const [results] = await connection.query(
      'SELECT PersonID, AccountID, NameOfUser FROM Accounts INNER JOIN Persons USING(PersonID) LIMIT ?, ?',
      [offset, rowLimit],
    );
...
const addNewFriend = async function (req, res, next) {
  ...
  try {
    ...
    console.log('existingFriendship', existingFriendship.length);
...
const increaseLikes = async function (res, req, next) {
  ...
  const connection = await pool.getConnection();
  try {
    const result = connection.query('CALL increase_likes(?)', [idOfPost]);
    if (result.affectedRows > 0) {
      res.send({success: true});
    } else {
      res.send({
        success: false,
        message: 'There is something wrong with the increaseLikes function.',
      });
    }
  } catch (error) {
    console.error(error);
...
const commentPost = async function (req, res, next) {
  ...
  try {
    ...
    const [commentInfor] = await connection.query(
      'INSERT INTO Comments(ContentOfComment) VALUES(?)',
      [ContentOfComment],
    );
    const CommentID = commentInfor.insertId;

    await connection.query(
      'INSERT INTO CommentSend(AccountID, CommentID, PostID, ToAccountID) VALUES(?, ?, ?, ?)',
    ...
    await res.send({
      success: true,
      message: 'commentPost function working perfectly.',
    });
  } catch (error) {
    await connection.rollback();
    res.status(500).json({success: false, message: 'Internal server error '});
...
const getCommentPost = async function (req, res, next) {
  ...
  const connection = await pool.getConnection();
  try {
    const [result] = await connection.query('CALL get_comment_of_post(?)', [
      PostID,
    ]);
    if (result.length > 0) {
      res.send({
        success: true,
        message: 'Get all comment of post successfully!',
        data: result[0],
      });
    } else {
      res.send({success: false, message: 'There is something wrong!'});
...
const getListChatFriend = async (req, res, next) => {
  ...
  try {
    const [result] = await connection.query('CALL get_all_friends(?)', [
      UserID,
    ]);
  ...
  try {
    ...
    for (elem of data) {
      ...
      await connection.query(
        ...
        [elem.idAccount, ChatContentID, elem.ToAccountID],
      );
    }
...
const getImageValidation = async (req, res, next) => {
  ...
  const pythonProcess = spawn('python', [
    'D:\\project\\facial_recognition\\predict_face.py',
  ]);
